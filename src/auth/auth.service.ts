...
export class AuthService {
  ...
  async register(
    ...
    if (existing) {
      if (existing.verificationInfo?.verified) {
        throw new BadRequestException('Email already registered');
      }

      // If user exists but is unverified, update details and resend verification OTP
      ...
      existing.phoneNum = phoneNum;
      existing.verificationInfo = {
        verified: false,
        token: otp,
      };
      await existing.save();
      ...
      if (autoGenerated) {
        await this.emailService.sendPasswordMail(email, password);
      }
      await this.emailService.sendRegistrationOtpMail(email, otp);

      const payload = {
        sub: (existing as unknown as { _id: string })._id,
        role: existing.role,
    ...
    // 2️⃣ Send emails with cleanup if sending fails
    try {
      if (autoGenerated) {
        await this.emailService.sendPasswordMail(email, password);
      }
      ...
      await this.userService.userModel.findByIdAndDelete(user._id);
      throw error;
    }
